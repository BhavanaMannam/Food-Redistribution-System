...
from .models import Tenant, User, InventoryItem, SurplusListing, Booking, SalesHistory, BarcodeProduct
from .schemas import (
    ...
    MatchingNGOResponse,
    BarcodeProductCreate, BarcodeProductUpdate, BarcodeProductResponse,
)
...
def create_surplus_listing(listing: SurplusListingCreate, tenant_id: int, db: Session = Depends(get_db)):
    # Allow business, ngo, and individual tenants
    tenant = db.query(Tenant).filter(Tenant.id == tenant_id).first()
    if not tenant:
        raise HTTPException(status_code=403, detail="Only registered tenants can list surplus food")
...
def create_booking(booking: BookingCreate, ngo_id: int, db: Session = Depends(get_db)):
    # Allow NGOs and Individuals (who donate directly to an NGO on their behalf)
    ngo = db.query(Tenant).filter(Tenant.id == ngo_id).first()
    if not ngo:
        raise HTTPException(status_code=403, detail="Recipient organization not found")
...
def get_my_donations(donor_id: int, db: Session = Depends(get_db)):
    """
    Returns all donation bookings sent by an individual donor, with NGO info and status.
    """
    bookings = db.query(Booking).join(SurplusListing, Booking.listing_id == SurplusListing.id).filter(
        SurplusListing.tenant_id == donor_id
    ).all()
    result = []
    for b in bookings:
        listing = b.listing
        ngo = db.query(Tenant).filter(Tenant.id == b.ngo_id).first()
        result.append({
            "booking_id": b.id,
            ...
            "ngo_phone": ngo.contact_phone if ngo else "",
        })
    result.sort(key=lambda x: x["created_at"], reverse=True)
    return result


@app.get("/api/v1/marketplace/incoming-donations")
def get_incoming_donations(ngo_id: int, db: Session = Depends(get_db)):
    """
    Returns bookings where an individual donated directly to this NGO.
    These are pending bookings the NGO needs to accept or decline.
    """
    ngo = db.query(Tenant).filter(Tenant.id == ngo_id).first()
    if not ngo:
        raise HTTPException(status_code=404, detail="NGO not found")

    bookings = db.query(Booking).filter(Booking.ngo_id == ngo_id).join(SurplusListing).filter(
        SurplusListing.tenant_id != ngo_id
    ).all()
...
def get_bookings(tenant_id: int, db: Session = Depends(get_db)):
    ...
    If Business/Individual: incoming requests on their listings (includes requesting NGO as ngo).
    """
    tenant = db.query(Tenant).filter(Tenant.id == tenant_id).first()
    if not tenant:
        raise HTTPException(status_code=404, detail="Tenant not found")
...
# ----------------- BARCODE PRODUCT CATALOGUE -----------------

@app.get("/api/barcodes", response_model=List[BarcodeProductResponse])
def list_barcode_products(db: Session = Depends(get_db)):
    """Return all barcode product records, newest first."""
    return db.query(BarcodeProduct).order_by(BarcodeProduct.id.desc()).all()


@app.post("/api/barcodes", response_model=BarcodeProductResponse, status_code=201)
def create_barcode_product(data: BarcodeProductCreate, db: Session = Depends(get_db)):
    """Add a new barcode product to the catalogue."""
    if db.query(BarcodeProduct).filter(BarcodeProduct.barcode == data.barcode).first():
        raise HTTPException(status_code=400, detail="Barcode already exists")
    product = BarcodeProduct(**data.model_dump())
    db.add(product)
    db.commit()
    db.refresh(product)
    return product
...
# NOTE: lookup by barcode STRING uses /lookup/ prefix to avoid collision with
# the numeric-ID routes below (FastAPI matches path params by order).
@app.get("/api/barcodes/lookup/{barcode}", response_model=BarcodeProductResponse)
def lookup_barcode_product(barcode: str, db: Session = Depends(get_db)):
    """Look up a single product by its barcode string."""
    product = db.query(BarcodeProduct).filter(BarcodeProduct.barcode == barcode).first()
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
    return product


@app.put("/api/barcodes/{product_id}", response_model=BarcodeProductResponse)
def update_barcode_product(product_id: int, data: BarcodeProductUpdate, db: Session = Depends(get_db)):
    """Update an existing barcode product by its numeric ID."""
    product = db.query(BarcodeProduct).filter(BarcodeProduct.id == product_id).first()
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
...
@app.delete("/api/barcodes/{product_id}")
def delete_barcode_product(product_id: int, db: Session = Depends(get_db)):
    """Delete a barcode product by its numeric ID."""
    product = db.query(BarcodeProduct).filter(BarcodeProduct.id == product_id).first()
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
    db.delete(product)
